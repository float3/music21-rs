...
function romanText(value: RomanNumeralInfo | null | undefined): string {
    ...
    return value.key_context
        ? `${value.figure} in ${value.key_context}`
        : value.figure;
}
...
function renderFacts(data: ChordAnalysis): void {
    const helpText: Record<string, string> = {
        "Forte class":
            "A catalog label for the chord's pitch-class set, grouping transpositionally or inversionally related sets.",
        "Normal form":
            "The chord's pitch classes rotated into their most compact ascending order.",
        "Interval-class vector":
            "A six-number count of the unordered interval classes contained inside the pitch-class set.",
    };
...
function renderFrequencyCell(pitch: PitchInfo): HTMLTableCellElement {
    ...
    for (const tuning of tuningFrequencies) {
        ...
        if (Math.abs(tuning.cents_from_equal_temperament) >= 0.005) {
            ...
            cents.textContent = formatCents(
                tuning.cents_from_equal_temperament,
            );
...
async function renderGuitarFingering(
    fingering: GuitarFingeringInfo | null,
): Promise<void> {
...
function renderGuitarUnavailable(
    node: HTMLElement,
    reason: unknown = null,
): void {
...
function normalizeVexChordsRenderer(module: unknown): VexRenderer | null {
    ...
    for (const candidate of [
        ...
        if (typeof candidateRecord.draw === "function") {
            return {
                draw: candidateRecord.draw.bind(
                    candidate,
                ) as VexRenderer["draw"],
            };
        }
        if (typeof candidateRecord.ChordBox === "function") {
            return {
                ChordBox: candidateRecord.ChordBox as VexChordBoxConstructor,
            };
        }
...
function vexChordForFingering(
    ...
    return {
        ...
        bars: barres,
        barres,
        tuning: (fingering.strings || []).map((string) =>
            vexTuningLabel(string.string_name),
        ),
    };
}
...
function vexBarresForFingering(
    ...
    for (const [key, strings] of groups) {
        ...
        const isContiguous = sorted.every(
            (string, index) => index === 0 || sorted[index - 1] - string === 1,
        );
...
function toggleKeyboardPitch(midi: number): void {
    if (isMidiChordInput(input.value)) {
        ...
        const hasPitch = tokens.some(
            (token) => midiNumberForInputToken(token) === midi,
        );
...
function showResolutionMotion(resolution: ResolutionChordInfo): void {
    ...
    try {
        resolutionAnalysis = analyzeWithCurrentOptions(
...
function loadChordHistory(): string[] {
    try {
        ...
        return values
            .map((value) =>
                typeof value === "string" ? normalizeChordInput(value) : "",
            )
            .filter((value) => {
...
function renderChordHistory(): void {
    ...
    for (const value of chordHistory) {
        const label =
            chordHistoryLabels.get(value) || historyLabelForChord(value);
        const option = document.createElement("option");
        option.value = value;
...
function historyLabelForChord(value: string): string {
    try {
        ...
        const label =
            data.pitched_common_names?.[0] || data.pitched_common_name || value;
...
function scheduleChordFrequencies(
    ...
    for (const frequency of frequencies) {
        ...
        oscillator.addEventListener("ended", () => {
            activeChordNodes = activeChordNodes.filter(
...
function arpeggioDuration(frequencies: number[]): number {
    ...
    return Math.max(
        noteDuration,
        (frequencies.length - 1) * step + noteDuration,
    );
}

function scheduleArpeggioFrequencies(
    frequencies: number[],
    startTime: number,
): void {
    const step = 0.14;
...
function scheduleFrequencyItem(
    item: FrequencySequenceItem,
    startTime: number,
): number {
...
async function playFrequencySequence(
    sequence: FrequencySequenceItem[],
): Promise<void> {
    ...
    for (const item of playable) {
        startTime +=
            scheduleFrequencyItem(item, startTime) + (item.gapAfter ?? 0);
    }
}
...
async function playResolutionPreview(
    resolution: ResolutionChordInfo,
): Promise<void> {
    if (!currentAnalysis && !analyze({ remember: false })) return;
    const analysis = currentAnalysis;
    if (!analysis) return;
    ...
    await playFrequencySequence([
        {
            frequencies: frequenciesFromAnalysis(analysis),
            duration: 1.0,
            gapAfter: 0.16,
        },
        {
            frequencies: frequenciesFromAnalysis(resolutionAnalysis),
            duration: 1.45,
        },
    ]);
}
...
async function startMidiInput(): Promise<void> {
    ...
    try {
        ...
        setMidiStatus(
            err instanceof Error ? err.message : "MIDI access denied",
        );
    }
}
...
function midiInputStatus(): string {
    ...
    const names = connectedMidiInputs().map(
        (inputDevice) => inputDevice.name || "MIDI input",
    );
...
function connectedMidiInputs(): MIDIInput[] {
    if (!midiAccess) return [];
    return midiInputs(midiAccess).filter(
        (inputDevice) => inputDevice.state !== "disconnected",
    );
}
...
function normalizeRandomNoteBounds(changed?: RandomBoundInput): {
    min: number;
    max: number;
} {
...
randomMinNotes.addEventListener("input", () =>
    normalizeRandomNoteBounds("min"),
);
randomMinNotes.addEventListener("change", () =>
    normalizeRandomNoteBounds("min"),
);
randomMaxNotes.addEventListener("input", () =>
    normalizeRandomNoteBounds("max"),
);
randomMaxNotes.addEventListener("change", () =>
    normalizeRandomNoteBounds("max"),
);
