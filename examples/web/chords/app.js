// Generated from app.ts. Edit the TypeScript source first.
import "../help-tooltips.js";
import init, { analyze_chord, known_chords } from "../pkg/music21_rs_web.js";

const search = mustQuery("#search");
const minCardinality = mustQuery("#min-cardinality");
const maxCardinality = mustQuery("#max-cardinality");
const root = mustQuery("#root");
const namedOnly = mustQuery("#named-only");
const count = mustQuery("#count");
const rows = mustQuery("#rows");
const error = mustQuery("#error");

let allChords = [];
let showNamedOnly = false;
const resolutionCache = new Map();
const inversionLabelCache = new Map();
...
function selectedRootPitchClass() {
  return Number(root.value) || 0;
}

function pitchNameAt(midi, names, withOctave) {
...
function realizedChord(chord, inversion = 0) {
...
function normalizedChordName(name) {
  return name.trim().replace(/\s+/g, " ").toLocaleLowerCase();
}

function displayInversionName(name) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/^\S/, (letter) => letter.toLocaleLowerCase());
}

function uniqueInversionName(chord, inversion) {
  ...
  try {
    const analysis = analyze_chord(`midi: ${realized.midi.join(" ")}`);
...
function inversionButtonLabel(chord, inversion) {
  return uniqueInversionName(chord, inversion) ?? inversionLabel(inversion);
}

function textSearch(chord) {
  return [
...
function renderCardinalityOptions(chords) {
...
function openUrl(chord, inversion = 0) {
...
function resolutionUrl(resolution) {
  const url = new URL(chordBaseHref, window.location.href);
  url.searchParams.set("chord", resolution.pitch_names.join(" "));
  return url.href;
}

function resolutionChords(chord) {
...
function renderResolutions(chord) {
  const wrap = document.createElement("div");
...
async function main() {
  try {
    await init();
    allChords = known_chords().map((chord) => ({
      ...chord,
