...
function generateRandomPolyrhythm() {
    const [trackMin, trackMax] = randomSettingRange(
        randomTrackMin,
        randomTrackMax,
        1,
        8,
            ...
            .map((value) => {
                try {
                    return typeof value === "string"
                        ? normalizePolyrhythmInput(value)
                        : "";
                } catch {
                    return "";
                }
...
function savePolyrhythmHistory() {
    try {
        window.localStorage?.setItem(
            historyStorageKey,
            JSON.stringify(polyrhythmHistory),
        );
    } catch {
...
function rememberPolyrhythm(value) {
    ...
    polyrhythmHistory = [
        normalized,
        ...polyrhythmHistory.filter(
            (historyValue) => historyValue !== normalized,
        ),
    ].slice(0, maxHistoryItems);
    savePolyrhythmHistory();
    renderPolyrhythmHistory();
...
function removeTrack(index) {
    ...
    mutedTracks = new Set(
        [...mutedTracks]
            ...
            .map((trackIndex) =>
                trackIndex > index ? trackIndex - 1 : trackIndex,
            ),
    );
    setPolyrhythmComponents(components);
}
...
function buildAnalysis() {
    ...
    const libraryAnalysis = analyzePolyrhythm(
        components,
        base,
        tempo,
        rootInput.value,
...
function renderCycle(data) {
    ...
    for (const [voiceIndex, component] of data.components.entries()) {
        ...
        row.title = isMuted
            ? "Click this track to unmute it"
            : "Click this track to mute it";
        ...
        number.setAttribute(
            "aria-label",
            `Subdivision for track ${voiceIndex + 1}`,
        );
...
async function renderChord(data) {
    ...
    renderChips(
        chordRatios,
        data.ratioTones.map((tone) => `${tone.component}`),
    );
...
function setActiveTick(tickIndex) {
    ...
    for (const tick of cycleGrid.querySelectorAll(
        `[data-tick="${tickIndex}"]`,
    )) {
...
function playVoice(index, accent) {
    ...
    oscillator.frequency.value = accent
        ? 880
        : voiceFrequencies[index % voiceFrequencies.length];
...
async function startPlayback() {
            ...
            const timer = window.setTimeout(
                () => {
                    ...
                    event.triggers.forEach((trigger, index) => {
                        if (trigger && !mutedTracks.has(index))
                            playVoice(index, event.tick === 0);
                    });
                },
    ...
    timers.push(
        window.setTimeout(stopPlayback, analysis.cycles * cycleMs + 160),
    );
}
