...
pub(crate) fn write(path: &Path, data: &TuningTables) -> Result<(), Box<dyn Error>> {
    ...
    out.push_str(
        "# Source of truth for the tuning-system ratio tables.\n\
         #\n\
         # Regenerate with `cargo run --release -p xtask -- regenerate-tuning-tables`, which\n\
         ...
         #\n\
         # Emit the Rust with `cargo run --release -p xtask -- emit-tuning-tables`.\n\n",
    );
    writeln!(
        out,
        "music21_version = {}",
...
pub(crate) fn render(data: &TuningTables) -> String {
    let mut out = String::new();
    out.push_str(
        "// @generated by `cargo run --release -p xtask -- emit-tuning-tables`.\n\
