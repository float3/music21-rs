...
use shared::{Tables, get_tables, init_py_with_dummies, prepare, run_command};
...
fn regenerate_tables(workspace_root: &Path) -> Result<(), Box<dyn Error>> {
    ...
    let generated_path = workspace_root.join("src/chord/tables/generated.rs");
    Python::attach(|py| -> PyResult<()> {
        init_py_with_dummies(py)?;
        let tables = get_tables(py)?;
        let rust = generate_rust_tables(py, &tables)?;

        fs::write(&generated_path, rust)
            .map_err(|err| PyErr::new::<PyIOError, _>(format!("{err}")))?;

        println!(
            "Rust chord tables regenerated at {}",
            generated_path.display()
        );
        Ok(())
    })?;

    let generated_path = generated_path
        .to_str()
        .ok_or_else(|| "generated table path is not valid UTF-8".to_string())?;
    run_command(&["rustfmt", generated_path], "rustfmt")?;
...
fn generate_inversion_default_pitch_class(tables: &Tables<'_>) -> PyResult<String> {
...
fn generate_forte_number_with_inversion_to_tn_index(tables: &Tables<'_>) -> PyResult<String> {
...
fn generate_tn_index_to_chord_info(tables: &Tables<'_>) -> PyResult<String> {
...
fn generate_rust_tables(py: Python<'_>, tables: &Tables<'_>) -> PyResult<String> {
