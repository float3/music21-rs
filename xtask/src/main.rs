...
fn print_help() {
    ...
    eprintln!("  cargo run --release -p xtask -- emit-tables");
    eprintln!("  cargo run --release -p xtask -- verify-tables");
    ...
    eprintln!("  cargo run --release -p xtask -- regenerate-fixtures");
    eprintln!("  cargo run --release -p xtask --features python -- regenerate-all");
    eprintln!(
        "  cargo run --release -p xtask -- report [--out DIR] [--coverage-only|--features-only|--suites-only|--benchmarks-only] [--no-suites] [--no-benchmarks] [--from-json FILE]"
    );
}
...
fn regenerate_fixtures(_: &Path) -> Result<(), Box<dyn Error>> {
    Err(
        "regenerate-fixtures requires xtask's python feature; run `cargo run --release -p xtask --features python -- regenerate-fixtures`"
            .into(),
    )
}
...
fn verify_scala_archive(workspace_root: &Path) -> Result<(), Box<dyn Error>> {
    ...
    if actual != expected {
        return Err(format!(
            "{} is out of date; run `cargo run --release -p xtask -- emit-scala-archive`",
            path.display()
        )
        .into());
    }
...
fn verify_tuning_tables(workspace_root: &Path) -> Result<(), Box<dyn Error>> {
    ...
    if actual != expected {
        return Err(format!(
            "{} is out of date; run `cargo run --release -p xtask -- emit-tuning-tables`",
            path.display()
        )
        .into());
    }
...
fn regenerate_tables(_: &Path) -> Result<(), Box<dyn Error>> {
    Err(
        "regenerate-tables requires xtask's python feature; run `cargo run --release -p xtask --features python -- regenerate-tables`"
            .into(),
    )
}
...
fn verify_tables(workspace_root: &Path) -> Result<(), Box<dyn Error>> {
    let data = read_table_data(&table_data_path(workspace_root))?;
    let expected = formatted_generated_rust(workspace_root, &data)?;
    let path = generated_path(workspace_root);
    let actual = fs::read_to_string(&path)?;

    if actual != expected {
        return Err(format!(
            "{} is out of date; run `cargo run --release -p xtask -- emit-tables`",
            path.display()
        )
        .into());
    }
    ...
    println!(
        "Committed Rust chord tables match {}",
        table_data_path(workspace_root).display()
    );
    Ok(())
}
...
fn emit_generated_rust(
    ...
    let path = generated_path(workspace_root);
    fs::write(&path, formatted_generated_rust(workspace_root, data)?)?;
    println!("Rust chord tables regenerated at {}", path.display());
    Ok(())
}

fn formatted_generated_rust(
    workspace_root: &Path,
    data: &ChordTablesData,
) -> Result<String, Box<dyn Error>> {
    let scratch_dir = workspace_root.join("target/xtask");
    fs::create_dir_all(&scratch_dir)?;
    let scratch_path = scratch_dir.join(format!("generated-{}.rs", std::process::id()));

    fs::write(&scratch_path, render_generated_rust(data))?;
    let rustfmt_path = scratch_path
        .to_str()
        .ok_or_else(|| "generated table scratch path is not valid UTF-8".to_string())?;
    run_command(&["rustfmt", rustfmt_path], "rustfmt generated chord tables")?;
...
fn render_generated_rust(data: &ChordTablesData) -> String {
    ...
    let mut rust = String::from(
        "/*\nThis file is autogenerated from data/chord_tables.toml.\nRegenerate it with `cargo run --release -p xtask -- emit-tables`.\nRefresh the TOML from music21 with `cargo run --release -p xtask --features python -- regenerate-tables`.\n*/\n\n",
    );
