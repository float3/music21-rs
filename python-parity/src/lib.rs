...
//! doctest harness in `tests/doctest_pitch.rs` imports the real music21,
//! replaces `music21.pitch.Pitch` (and `Accidental`, `Microtone`) with these,
//! and runs the docstrings of `music21/pitch.py` as they are. What passes is
//! what the crate reproduces to the letter; what fails is either a fidelity
//! gap or a feature the crate does not have.
//!
...
/// it.
fn message(error: &music21_rs::Error) -> String {
...
fn interval_from_any(value: &Bound<'_, PyAny>) -> PyResult<Interval> {
...
fn pitch_from_any(value: &Bound<'_, PyAny>) -> PyResult<RsPitch> {
...
impl Pitch {
    fn wrap(inner: RsPitch, spelling_is_inferred: bool) -> Self {
...
#[allow(non_snake_case)]
fn simplify_multiple_enharmonics(
...
fn convert_pitch_class_to_str(pitch_class: i32) -> String {
...
fn is_valid_accidental_name(name: &str) -> bool {
...
fn standardize_accidental_name(name: &str) -> PyResult<String> {
...
/// The Python module: `import music21_rs_facade`.
#[pymodule]
