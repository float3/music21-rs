    ...
    for card in range(len(tables.FORTE)):
        ...
        for i, entry in enumerate(card_data):
            if entry is None:
                ...
                rust_code += f"\n            Some(TNIStructure{{ pitch_classes:{pcs_vec}, interval_class_vector: {icv_vec}, invariance_vector: {inv_vec_vec}, z_relation: {z_rel}}}),\n"
        rust_code += "        ],\n"
    rust_code += "    ]});\n"
...
def generate_inversion_default_pitch_class():
    rust_code = "\n    static INVERSION_DEFAULT_PITCH_CLASSES: LazyLock<HashMap<(u8, u8), Vec<u8>>> = LazyLock::new(|| {"
...
def generate_maximum_index_number_without_inversion_equivalence():
    rust_code = "\nstatic MAXIMUM_INDEX_NUMBER_WITHOUT_INVERSION_EQUIVALENCE: LazyLock<Vec<u8>> = LazyLock::new(|| vec!["
...
def generate_maximum_index_number_with_inversion_equivalence():
    rust_code = "\n\nstatic MAXIMUM_INDEX_NUMBER_WITH_INVERSION_EQUIVALENCE: LazyLock<Vec<u8>> = LazyLock::new(|| vec!["
...
def generate_tn_index_to_chord_info():
    if NoneMode:
        ...
        rust_code = "\n    static TN_INDEX_TO_CHORD_INFO: LazyLock<HashMap<U8U8SB, Vec<&'static str>>> = LazyLock::new(|| {"
