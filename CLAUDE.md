...
**Ported music21 tables**: several music21 data tables are mirrored here,
and the ones that are hand-maintained are checked against upstream by
`python-parity` fixtures generated by
`python-parity/generate_scale_expectations.py`:

| music21 | here | verified by |
| --- | --- | --- |
| `harmony.CHORD_TYPES` (50) | `chordsymbol::MUSIC21_CHORD_TYPES` | `chord_type_parity` |
| `ConcreteScale` subclasses (19) | `scale::ScaleType` | `scale_parity` |
| `duration.typeToDuration` (17) | `duration::DurationType` | in-module test |
| `pitch.accidentalNameToModifier` (13) | `pitch::accidental` | — |
| `key.modeSharpsAlter` (9) | `key::keysignature` | — |
| `interval.prefixSpecs` (12) | `interval::Specifier` | — |
| `chord/tables.py` | `chord::tables` (generated) | `xtask verify-tables` |
| Scala `.scl` archive | `tuningsystem::scala`, tuning tables | `scala_archive`, `verify-tuning-tables` |

`harmony.CHORD_ALIASES` (3) is deliberately absent: it maps input
spellings onto kinds, and the crate's table is only used to *write*
figures, so there is nowhere for an alias to apply yet. A test asserts
every alias target exists, so adding kind-by-name parsing later cannot
start out broken.
